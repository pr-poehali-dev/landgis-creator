...
const Index = () => {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      <div className="w-96 border-r border-border flex flex-col bg-card/50 backdrop-blur">
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-16 border-b border-border flex items-center justify-between px-6 bg-card/30 backdrop-blur">
          <Tabs value={mapType} onValueChange={(v) => setMapType(v as 'scheme' | 'hybrid')}>
            <TabsList>
        ...
        <div className="h-20 border-t border-border bg-card/30 backdrop-blur px-6 flex items-center justify-between">
          <div className="flex gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">Доступно: <span className="font-semibold text-foreground">{mockProperties.filter(p => p.status === 'available').length}</span></span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
              <span className="text-muted-foreground">Резерв: <span className="font-semibold text-foreground">{mockProperties.filter(p => p.status === 'reserved').length}</span></span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-gray-500"></div>
              <span className="text-muted-foreground">Продано: <span className="font-semibold text-foreground">{mockProperties.filter(p => p.status === 'sold').length}</span></span>
            </div>
          </div>
